...
if __name__ == "__main__":
    ...
    optimizer_g = torch.optim.Adam(
        gen.parameters(), lr=opts.lr, betas=(opts.beta1, opts.beta2))
    optimizer_d = torch.optim.Adam(
        disc.parameters(), lr=opts.lr, betas=(opts.beta1, opts.beta2))
    ...
    for epoch in range(start_epoch, opts.numEpochs):
        ...
        for batch_num, images in enumerate(progress_bar):
            ...
            fake_image_labels_d = torch.rand(images.shape[0]).cuda() / 10 + 0.9
            valid_image_labels_d = torch.rand(images.shape[0]).cuda() / 10
